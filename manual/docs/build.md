...
不需要同项目集其他目录。使用 `cjpm bundle` 生成 target/mapsdk-0.2.0.cjp，包含源码/测试、公开manual、examples、LICENSE与NOTICE。

仓外消费可使用 Git 依赖，锁定经验证的提交：

```toml
[dependencies]
mapsdk = { git = "https://github.com/Celading/mapsdk.git", branch = "main" }
```
